"""Modelling code for Looped Gemma2."""
...
from transformers import AutoTokenizer, Gemma2ForCausalLM, Gemma2Model
from transformers.cache_utils import Cache, DynamicCache
from transformers.masking_utils import create_causal_mask, create_sliding_window_causal_mask
from transformers.modeling_outputs import BaseModelOutputWithPast
from transformers.models.gemma2.configuration_gemma2 import Gemma2Config
from transformers.models.gemma2.modeling_gemma2 import Gemma2DecoderLayer, logger
...
CACHE_DIR = os.path.join(os.environ.get("SCRATCH", "/SCRATCH"), "gemma-exps")
...
# Model
class HackedGemma2DecoderLayer(Gemma2DecoderLayer):
    ...
    def forward(self, *args, **kwargs):
        ...
        if _layer_idx is not None:
            ...
            layer_idx = _layer_idx["layer_idx"]
            virtual_layer_idx = _layer_idx["virtual_layer_idx"]

            # Cache fixes
            self.self_attn.layer_idx = virtual_layer_idx

            # Local/Global attention fixes
            self.is_sliding = not bool(layer_idx % 2)
            self.self_attn.sliding_window = self.self_attn.config.sliding_window if not bool(layer_idx % 2) else None

        return super().forward(*args, **kwargs)


class HackedGemma2Model(Gemma2Model):
    def __init__(self, config: Gemma2Config):
        super().__init__(config)

        self.layers = nn.ModuleList([HackedGemma2DecoderLayer(config, i) for i in range(config.num_hidden_layers)])
    ...
    def forward(  # noqa: C901, PLR0912, PLR0915
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if use_cache and past_key_values is None and not self.training:
            ...
            if self.config.exec_plan is not None:
                copied_config = deepcopy(self.config)
                copied_config.num_hidden_layers = len(self.config.exec_plan)

            past_key_values = DynamicCache(config=copied_config)
    ...
    def _generate_weights(
        ...
        if interpolation_mode == "no":
            ...
            raise NotImplementedError(
                f"Interpolation mode '{interpolation_mode}' is not implemented. Supported: ['no', 'uniform', 'ema', 'auto-alignment', path]",  # noqa: E501
            )
...
def auto_alignment_interpolation(cache: torch.Tensor, temperature: float = 1.0) -> torch.Tensor:
    """
...
def load_hacked_model(
...
) -> tuple[AutoTokenizer, Gemma2ForCausalLM]:
    if plan is not None and plan != AUTO_PLAN:
        print("Loading Hacked Gemma2 Model...")
        print("Using execution plan:", plan)
    elif plan is None:
        print("Loading Vanilla Gemma2 Model...")

    extra_args = {"attn_implementation": "eager"} if train else {}

    tokenizer = AutoTokenizer.from_pretrained(model_id, cache_dir=CACHE_DIR)
    model = Gemma2ForCausalLM.from_pretrained(model_id, cache_dir=CACHE_DIR, **extra_args)
    ...
    if plan is not None:
        ...
        model.model = HackedGemma2Model.from_pretrained(model_id, cache_dir=CACHE_DIR, **extra_args)
        ...
        model._no_split_modules = ["HackedGemma2DecoderLayer"]
        model.tie_weights()  # important for training with FSDP2
...
if __name__ == "__main__":
    ...
    print(model.model.interpolation_mode)

    exec_plan = model.model.config.exec_plan or range(model.model.config.num_hidden_layers)
    print(f"Exec plan: {format_exec_plan(exec_plan)}")
